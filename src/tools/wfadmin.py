...
def int_or_hex(string):	
	try:
		...
		raise argparse.ArgumentTypeError('{} is not an integer!'.format(string))
	
def byte_literal(string):
	res = [0]*64
	
	if len(string) % 2:
		raise argparse.ArgumentTypeError('{} has an uneven number of nibbles!'.format(string))
	else:
		i = 0
		while (string):
	...
	def __call__(self, parser, namespace, values, option_string=None):
		with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as s:
			...
			if type(values) == type(1):
				...
				l = values[0]
				values = values[1:]
				# CMD, 1 byte length, 64 bytes symbols, first symbol, last symbol
			...
			try:
				...
				if cmd == self.cmd and flags & WIFIRE_REPLY_SUCCESS:
					print(self.msgs['success'].format(namespace.USRP2_IP, values))
				else:
					print('<-- WiFire (@{}): Unexpected reply cmd: {} flags: {}"!'.format(namespace.USRP2_IP, cmd, flags))
...
class PeriodicAction(WiFireAction):
	def __init__(self, **kwds):
		...
		self.msgs = {'req':'--> WiFire (@{}): Changing periodic to {}.',
					 'success':'<-- WiFire (@{}): Setting Periodic to {} successful!'}	
...
class TXScaleAction(WiFireAction):
	def __init__(self, **kwds):
		...
		self.msgs = {'req':'--> WiFire (@{}): Changing TX scale to {:=#010x}.',
					 'success':'<-- WiFire (@{}): Setting TX Scale to {:=#010x} successful!'}
...
class RDelayAction(WiFireAction):
	def __init__(self, **kwds):
		...
		self.msgs = {'req':'--> WiFire (@{}): Changing reaction delay to {} ticks.',
					 'success':'<-- WiFire (@{}): Setting reaction delay to {} ticks successful!'}
...
class SymbolsAction(WiFireAction):
	def __init__(self, **kwds):
		...
		self.msgs = {'req':'--> WiFire (@{}): Changing TX symbols to {}.',
					 'success':'<-- WiFire (@{}): Setting TX symbols to {} successful!'}	
			

if __name__ == '__main__':
	p = argparse.ArgumentParser(description='WiFire administration tool (python style).')
	...
	p.add_argument('--opmode', action=OpmodeAction, type=opmode,
                   help='WiFire Opmode to send to the USRP2 [idle, react, tx_zigbee].')
	p.add_argument('--periodic', action=PeriodicAction, default=50000000, type=int,
                   help='Periodic interrupt that triggers actions; specify the number of clock cycles to wait, '
                   'clock is 50MHz.')
	p.add_argument('--tx_scale', action=TXScaleAction, default=0x04000400, type=int_or_hex,
	...
	p.add_argument('--syms', action=SymbolsAction, default='', type=byte_literal,
				help='Specify the packet to be put into the TX symbol buffer.')
