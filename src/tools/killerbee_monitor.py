...
class RZVictim(object):
    def __init__(self, channel=26):
        ...
        print "RZVictim: listening on \'%s\', link-type DLT_IEEE802_15_4, capture size 127 bytes" % self.kb.get_dev_info()[0]
...
try:
	...
	if packet[1] and packet[3] and packet[5]:	# has src, dst and seqno
	    ...
	    try:
		data = unpack('=BHH', packet[1]+packet[3]+packet[5])
	    except struct.error:
	    ...
	    log('%d(%s) -> %d(%s) (#%d): Len: %d, Payload-Len: %d, FCF:%s, Payload:%s, crc:%s' % (src, repr(packet[4]), dst, repr(packet[2]), seq, len(p[0]), len(packet[7]), repr(packet[0]), repr(packet[7]), crc_state))
	...
	if packet[7][:2] == '\x3F\x70':    # ctp routing frame
	    ...
	    log('  CTP Routing: option:%d, parent:%d, etx:%d' % (option, parent, etx))
	elif packet[7][:2] == '\x3F\x71':   # ctp data frame
	    try:
		data = unpack('!BBHHBB', packet[7][2:10])
	    except struct.error:
	    ...
	    log('  CTP Data: option:%d, thl:%d, etx:%d, origin:%d, seqno:%d, collect_id:%d, data:%s' % (option, thl, etx, origin, seqno, collect_id, repr(payload)))
	elif packet[7][:2] == '\x3F\x73':	# lqi routing frame
	    try:
		data = unpack('!HHHHHH', packet[7][2:14])
	    except struct.error:
	    ...
	    cost = data[4]
	    hopcount = data[5]
	    log('  LQI routing: origin:%d, seqno:%d, originseqno:%d, parent:%d, cost:%d, hopcount:%d' % (originaddr, seqno, originseqno, parent, cost, hopcount))
	elif packet[7][:2] == '\x3F\x74':	# lqi data frame
	    try:
		data = unpack('!HHHHBH', packet[7][2:13])
	    except struct.error:
	    ...
	    collectionid = data[4]
	    payload = data[5]
	    log('  LQI data: origin:%d, seqno:%d, originseqno:%d, hopcount:%d, collect_id:%d, data:%d' % (originaddr, seqno, originseqno, hopcount, collectionid, payload))
	elif packet[7][:2] == '\x3F\xAA':	# measurement information
	    try:
		data = unpack('!IHHHH', packet[7][2:14])
	    except struct.error:
	    ...
	    packetcount = data[3]
	    missedcount = data[4]
	    log('  Measurement packet: framecount:%d, packetcount:%d, missedcount:%d, timediff:%d, duplicates:%d' % (framecount, packetcount, missedcount, timediff, duplicates))
